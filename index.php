...
<?php
	...
	require_once __DIR__.'/admin/includes/managers/shelf_manager.php';
	...
	if($db){
		...
		while($b = mysql_fetch_array($qry))
		{
			...
			$tapbeers[$b['tapNumber']] = $beeritem;
		}

		$sql =  "SELECT * FROM vwGetActiveShelves";
		$qry = mysql_query($sql);
		while($b = mysql_fetch_array($qry))
		{
			$beeritem = array(
			...
			$bottlebeers[$b['shelfNumber']] = $beeritem;
		}
...
<html>
	...
	<body>
		<div class="bodywrapper">
			...
			<!-- Bottle Header with Brewery Logo and Project Name -->
			<div class="header clearfix">
				<div class="HeaderLeft">
					<?php if($config[ConfigNames::UseHighResolution]) { ?>			
						<a href="admin/admin.php"><img src="<?php echo $config[ConfigNames::LogoUrl]; ?>" height="200" alt=""></a>
				...
				<div class="HeaderCenter">
					<h1 id="HeaderTitle">
						<?php
							if (mb_strlen($config[ConfigNames::BottleHeaderText], 'UTF-8') > ($config[ConfigNames::HeaderTextTruncLen])) {
								$headerTextTrunced = substr($config[ConfigNames::BottleHeaderText],0,$config[ConfigNames::HeaderTextTruncLen]) . "...";
								echo $headerTextTrunced ; }
							else
			...
			<table>
				...
				<tbody>
					<?php for($i = 1; $i <= $numberOfShelves; $i++) {
						if( isset($bottlebeers[$i]) ) {
							$beer = $bottlebeers[$i];
					?>
							<tr class="<?php if($i%2 > 0){ echo 'altrow'; }?>" id="<?php echo $beer['id']; ?>">
								<?php if($config[ConfigNames::ShowTapNumCol]){ ?>
