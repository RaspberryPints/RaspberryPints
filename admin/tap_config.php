...
echo 'On<input type="radio" ' . ($tapconfig ["valveOn"] ? 'checked' : '') . ' name="valveon" value="1">' . '									Off<input type="radio" ' . (! $tapconfig ["valveOn"] ? 'checked' : '') . ' name="valveon" value="0"><br>';
									?>
											</td>
											<?php } ?>
											
											<td><input name="saveTapConfig" type="submit" class="btn"
								value="Save Tap Config" /></td>
						</tr>
								<?php } else { ?>
										<input type="hidden" name="tapNumber" value="<?php echo $c?>" />
						<tr>
...
if ($config [ConfigNames::UseTapValves]) {
									?>
											<td><input type="text" id="valvepin" class="mediumbox"
								name="valvepin" value="0" /></td>
							<td>Off</td>
											<?php } ?>
											
											<td colspan="99"><input name="saveTapConfig" type="submit"
								class="btn" value="Save Tap Config" /></td>
						</tr>								
								<?php } ?>	
							</form>						
						<?php } ?>
		...
		<p>
			...
			<br /> Interval is the interval with which the fan will be triggered
			(every x minutes, turn the fan on). <br /> Duration is the time the
			fan will run after it has been triggered.<br> If Duration is bigger
				that Interval, the fan always runs, if Duration is zero or less, the
				fan never runs. 
